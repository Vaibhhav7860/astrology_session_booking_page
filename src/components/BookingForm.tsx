...
export function BookingForm({ initialBasePrice }: { initialBasePrice: number }) {
    ...
    const [priceData, setPriceData] = useState({ amount: initialBasePrice, currency: "AED", loading: false });
    ...
    useEffect(() => {
        if (formData.country_code === "AED") {
            setPriceData({ amount: initialBasePrice, currency: "AED", loading: false });
            return;
        }
        const fetchConversion = async () => {
            ...
            try {
                const res = await fetch("/api/currency/convert", {
                    ...
                    body: JSON.stringify({ amount_aed: initialBasePrice, target_currency: formData.country_code })
                });
        ...
        fetchConversion();
    }, [formData.country_code, initialBasePrice]);
